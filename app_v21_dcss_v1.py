...
def analyse(df_raw):
    ...
    if kb_available:
        ...
        # Qualifies ANY incident that appears more than once in the tagged rows,
        # whether that is:
        #   (a) 2+ DIFFERENT tag types  e.g. RKM Solution + Known Error
        #   (b) 2+ rows of the SAME tag type  e.g. RKM Solution + RKM Solution
        #       (same incident referenced by two different KB articles)
        # Uses df_tag (all rows). If no Incident_Number column, skip.
        multi_tag_grp_rows = []
        if inc_col in df_tag.columns:
            # Only consider rows that carry one of the three known tag types
            tagged_rows = df_tag[mask_any].copy()
            tagged_rows[inc_col] = tagged_rows[inc_col].astype(str)

            # Count tag rows per incident (regardless of type)
            row_counts = tagged_rows.groupby(inc_col).size()

            # Also build type-set map for the "Tags" display column
            tag_label_map = {}
            for mask, label in [(mask_rkm, "RKM Solution"),
                                 (mask_ke,  "Known Error"),
                                 (mask_ps,  "Problem Investigation")]:
                for iid in df_tag.loc[mask, inc_col].dropna().astype(str).unique():
                    tag_label_map.setdefault(iid, set()).add(label)

            # Qualify: 2+ tag rows (catches same-type duplicates too)
            multi_ids = set(row_counts[row_counts >= 2].index)

            if multi_ids:
                # One representative row per incident (first occurrence)
                multi_df = (
                    tagged_rows[tagged_rows[inc_col].isin(multi_ids)]
                    .drop_duplicates(subset=[inc_col], keep="first")
                    .copy()
                )

                # Build display label:
                # - 2+ different types → "Known Error + RKM Solution"
                # - same type repeated → "RKM Solution (×3)"
                def _tag_label(iid):
                    types = tag_label_map.get(iid, set())
                    total = int(row_counts.get(iid, 1))
                    if len(types) >= 2:
                        return " + ".join(sorted(types))          # different types
                    elif types:
                        t = next(iter(types))
                        return f"{t} (×{total})" if total > 1 else t  # same type repeated
                    return f"Tagged ×{total}"

                multi_df["Tags"] = multi_df[inc_col].map(_tag_label)
                multi_df["Tag_Count"] = multi_df[inc_col].map(
                    lambda iid: int(row_counts.get(iid, 1))
                )
                # Sort: most tag rows first
                multi_df = multi_df.sort_values("Tag_Count", ascending=False)
                ...
                log.info("Multi-tagged incidents: %d IDs (%d diff-type, %d same-type) across %d groups",
                         len(multi_ids),
                         sum(1 for iid in multi_ids if len(tag_label_map.get(iid,set())) >= 2),
                         sum(1 for iid in multi_ids if len(tag_label_map.get(iid,set())) < 2),
                         len(multi_tag_grp_rows))
