...
def analyse(df_raw):
    ...
    kb_group_rows = []; kb_no_kb_rows = []; kb_summary_data = {}
    ...
    if kb_available:
        ...
        rkm_df = df[rkm_mask]; no_kb_df = df[~rkm_mask]
        kb_summary_data = {"total_rkm":len(rkm_df),"total_no_kb":len(no_kb_df),
        ...
        if F["AssignedGroup"]:
            for gname, gdf in df.groupby("AssignedGroup"):
                ...
                kb_group_rows.append({"group":gname,"total":len(gdf),"with_kb":len(grp_rkm),
                                       "without_kb":len(grp_nokb),"top_kbs":kb_ids.to_dict("records"),
        ...
        kb_no_kb_rows = no_kb_df[no_cols].head(200).fillna("—").to_dict("records")
...
<div id="dash">
  ...
  <section class="page" id="pg-kb">
    ...
    <div class="full cc"><div class="cc-hd"><span class="cc-dot" style="background:var(--purple)"></span>
      <span class="cc-title">KB Coverage by AssignedGroup</span>
      <span style="font-size:.68rem;color:var(--dim);margin-left:7px">Request_Type01 = RKM Solution</span></div>
      <div class="tbl-wrap"><table><thead><tr><th>AssignedGroup</th><th>Total</th><th>With KB</th><th>Without KB</th><th>KB %</th><th>Top KB IDs</th><th>Coverage</th></tr></thead>
      <tbody id="kb-tbody"></tbody></table></div>
    </div>
    <div class="full al-card">
...
function buildKB(d){
  ...
  $('kb-kpis').innerHTML=[
    {l:'RKM Solution Tagged',v:s.total_rkm||0,   c:'var(--purple)',i:'📚',sub:'Request_Type01 = RKM Solution'},
    {l:'Without KB Article', v:s.total_no_kb||0,  c:'var(--red)',  i:'❌',sub:'No KB tag detected'},
    {l:'Unique KB IDs',      v:s.unique_kb_ids||0,c:'var(--green)',i:'🔑',sub:'Distinct KB articles used'},
