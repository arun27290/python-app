...
# SLA colour map — GREEN is reserved ONLY for “Met” variants.

# Every other status gets its own distinct colour so nothing looks “good” by accident.

SLA_COLORS = {
# ── Met / compliant → GREEN ───────────────────────────────────────────────
“Met”:           GREEN,   “met”:           GREEN,
“Within SLA”:    GREEN,   “within sla”:    GREEN,
“SLA Met”:       GREEN,   “sla met”:       GREEN,
“Compliant”:     GREEN,   “compliant”:     GREEN,
“OK”:            GREEN,   “ok”:            GREEN,
# ── Breached / violated → RED ─────────────────────────────────────────────
“Breached”:      RED,     “breached”:      RED,
“SLA Breached”:  RED,     “sla breached”:  RED,
“Violated”:      RED,     “violated”:      RED,
...
# ── Invalid / not applicable → ORANGE ────────────────────────────────────
“Invalid”:       ORANGE,  “invalid”:       ORANGE,
“N/A”:           ORANGE,  “n/a”:           ORANGE,
“Not Applicable”:ORANGE,  “not applicable”:ORANGE,
“Cancelled”:     ORANGE,  “cancelled”:     ORANGE,
“Canceled”:      ORANGE,  “canceled”:      ORANGE,
# ── Exempt / excluded → MUTED SLATE ──────────────────────────────────────
“Exempt”:        “#94a3b8”,“exempt”:        “#94a3b8”,
“Excluded”:      “#94a3b8”,“excluded”:      “#94a3b8”,
“Skipped”:       “#94a3b8”,“skipped”:       “#94a3b8”,
}

# Fallback lookup: normalise to lowercase before checking SLA_COLORS

def sla_color(label):
“”“Return the colour for any SLA status label.
Tries exact match first, then lowercase, then a keyword scan,
then defaults to BLUE (neutral — never GREEN or RED by accident).
“””
if label in SLA_COLORS:          return SLA_COLORS[label]
ll = str(label).strip().lower()
if ll in SLA_COLORS:             return SLA_COLORS[ll]
# Keyword scan for values not explicitly listed
if any(k in ll for k in (“met”,“within”,“compliant”,“ok”,“yes”,“pass”)):
return GREEN
if any(k in ll for k in (“breach”,“miss”,“violat”,“fail”,“overdue”,“no “)):
return RED
if any(k in ll for k in (“pend”,“progress”,“open”)):
return YELLOW
if any(k in ll for k in (“invalid”,“n/a”,“cancel”,“not app”)):
return ORANGE
if any(k in ll for k in (“exempt”,“exclud”,“skip”)):
return “#94a3b8”
return BLUE   # unknown value → neutral blue, never green
...
def analyse(df_raw):
df_full = normalise_columns(df_raw.copy())   # full frame — may have duplicate incident rows

```
for col in ["ReportedDate","LastResolvedDate","SubmitDate"]:
    if col in df_full.columns:
        df_full[col] = parse_date_col(df_full[col])

F = {k: k in df_full.columns for k in [
...
inc_col = "Incident_Number" if "Incident_Number" in df_full.columns else df_full.columns[0]

# ── Deduplication (Option A) ───────────────────────────────────────────────
# df_tag  = full frame — used ONLY in Incident Tagging analysis (every tag row counts)
# df      = deduplicated by Incident_Number — used for ALL other metrics
# If no Incident_Number column exists, df == df_full (no dedup possible).
df_tag = df_full.copy()
if "Incident_Number" in df_full.columns:
    raw_rows  = len(df_full)
    # Keep the first occurrence of each incident for general metrics
    # (first row usually has the most complete Status/Priority/SLA data)
    df = df_full.drop_duplicates(subset=["Incident_Number"], keep="first").copy()
    dedup_dropped = raw_rows - len(df)
    if dedup_dropped:
        log.info("Dedup: %d raw rows → %d unique incidents (%d duplicate rows removed)",
                 raw_rows, len(df), dedup_dropped)
else:
    df = df_full.copy()
    dedup_dropped = 0

dedup_note = (
    f"Raw rows: {len(df_full):,} — Unique incidents after dedup: {len(df):,} "
    f"({dedup_dropped:,} duplicate rows removed). "
    f"Incident Tagging tab uses all {len(df_full):,} tag rows."
...
# Month_Resolved — always based on LastResolvedDate, used ONLY for MoM comparison.
# This ensures MoM groups incidents by when they were CLOSED, not when reported.
if F["LastResolvedDate"] and df["LastResolvedDate"].notna().any():
    df["Month_Resolved"] = df["LastResolvedDate"].dt.to_period("M").astype(str)
    log.info("Month_Resolved: %d values from LastResolvedDate (for MoM comparison)",
             df["Month_Resolved"].notna().sum())
else:
    df["Month_Resolved"] = df["Month"] if "Month" in df.columns else None
    log.info("Month_Resolved: LastResolvedDate not available, falling back to Month (%s)", date_ref)
...
# ── Resolver Scorecard / FTF per assignee ────────────────────────────────
resolver_rows = []
if F["AssignedGroup"] and F["Assignee"]:
    ...
    for gname, gdf in df.groupby("AssignedGroup"):
        for aname, adf in gdf.groupby("Assignee"):
            a_tot  = len(adf)
            a_res  = int(adf["Status"].str.strip().str.lower().isin(CLOSED_STATUSES).sum()) if F["Status"] else a_tot
            a_mttr = round(float(adf["MTTR_Hours"].mean()), 1) if adf["MTTR_Hours"].notna().any() else None
            a_sla  = float(round(adf["SLAStatus"].str.strip().str.lower().isin(MET_VALUES).sum() / a_tot * 100, 1)) if F["SLAStatus"] else None
            a_xf   = round(float(adf["Group_Transfers"].mean()), 1) if F["Group_Transfers"] and adf["Group_Transfers"].notna().any() else None
            a_ftf  = int((pd.to_numeric(adf["Group_Transfers"], errors="coerce") == 0).sum()) if F["Group_Transfers"] else 0
            a_ftf_pct = round(a_ftf / a_tot * 100, 1) if a_tot else 0
            all_assignees.append({
                "AssignedGroup": gname, "Assignee": aname,
                "Total":    a_tot,    "Resolved":  a_res,
                "SLA_Pct":  a_sla,   "Avg_MTTR":  a_mttr,
                "FTF":      a_ftf,   "FTF_Pct":   a_ftf_pct,
                "Avg_Hops": a_xf,
            })
    # Rank by total descending; assign percentile tiers
    all_assignees.sort(key=lambda x: x["Total"], reverse=True)
    n = len(all_assignees)
    for i, row in enumerate(all_assignees):
        pct_rank = (n - i) / n
        row["Tier"] = "🏆 Top" if pct_rank > 0.75 else ("✅ Good" if pct_rank > 0.50 else ("⚠ Mid" if pct_rank > 0.25 else "🔴 Low"))
        row["Rank"] = i + 1
    resolver_rows = all_assignees

# ── Workload distribution by Priority per Assignee (per group) ────────────
workload_by_group = []
if F["AssignedGroup"] and F["Assignee"] and F["Priority"]:
    # Map every priority variant to a short key P1/P2/P3/P4
    # Uses the same priority_sort_key logic so "Critical", "P1 - Critical", "P1" all → P1
    def _pri_bucket(val):
        k = priority_sort_key(val)
        return {0: "P1", 1: "P2", 2: "P3", 3: "P4"}.get(k, "Other")

    # Add a bucketed column to avoid relying on exact priority strings
    df["_pri_bucket"] = df["Priority"].apply(_pri_bucket)

    for gname, gdf in df.groupby("AssignedGroup"):
        # Pivot on bucket column — always produces P1/P2/P3/P4 columns
        wl_pivot = (
            gdf.groupby(["Assignee", "_pri_bucket"])
               .size()
               .unstack(fill_value=0)
               .reset_index()
        )
        # Ensure all bucket columns exist even if no incidents of that priority
        for p in ["P1", "P2", "P3", "P4"]:
            if p not in wl_pivot.columns:
                wl_pivot[p] = 0
        wl_pivot["Total"] = (
            wl_pivot.get("P1", 0) + wl_pivot.get("P2", 0) +
            wl_pivot.get("P3", 0) + wl_pivot.get("P4", 0)
        )
        wl_pivot = wl_pivot.sort_values("Total", ascending=False)
        rows_out = []
        for _, row in wl_pivot.iterrows():
            rows_out.append({
        ...
        workload_by_group.append({"group": gname, "rows": rows_out})

    # Clean up temporary column
    df.drop(columns=["_pri_bucket"], errors="ignore", inplace=True)
    workload_by_group.sort(key=lambda x: sum(r["Total"] for r in x["rows"]), reverse=True)

# ── Month-over-Month comparison ───────────────────────────────────────────
mom_data = {}
# MoM uses Month_Resolved (LastResolvedDate-based) — groups incidents by closure month
_mom_col = "Month_Resolved" if "Month_Resolved" in df.columns else "Month"
if _mom_col in df.columns and df[_mom_col].notna().any():
    months_sorted = sorted(df[_mom_col].dropna().unique())
    if len(months_sorted) >= 2:
        cur_m  = months_sorted[-1]
        prev_m = months_sorted[-2]
        # Also find same month last year if available
        try:
            cur_dt   = pd.Period(cur_m, "M")
            lyr_m    = str(cur_dt - 12)
            has_lyr  = lyr_m in months_sorted
        except Exception:
            lyr_m = None; has_lyr = False

        log.info("MoM using column '%s': prev=%s cur=%s lyr=%s",
                 _mom_col, prev_m, cur_m, lyr_m if has_lyr else "N/A")

        def _month_stats(mdf):
            tot  = len(mdf)
            sla  = float(round(mdf["SLAStatus"].str.strip().str.lower().isin(MET_VALUES).sum()/tot*100,1)) if F["SLAStatus"] and tot else None
            mttr_v = float(round(float(mdf["MTTR_Hours"].mean()),1)) if mdf["MTTR_Hours"].notna().any() else None
            p1_v = int(mdf["Priority"].str.lower().str.contains("critical|p1",na=False).sum()) if F["Priority"] else 0
            ftf_v = int((pd.to_numeric(mdf.get("Group_Transfers",pd.Series(dtype=float)),errors="coerce")==0).sum()) if F["Group_Transfers"] else 0
            ftf_pct = float(round(ftf_v/tot*100,1)) if tot else None
            # Incident Aging — resolved incidents that took >30 days from submit/report to resolve
            aging_ct = None
            if "AgeDays" in mdf.columns:
                aging_ct = int((mdf["AgeDays"] > 30).sum())
            elif "MTTR_Hours" in mdf.columns:
                aging_ct = int((mdf["MTTR_Hours"] > 720).sum())  # 720h = 30 days
            return {"total":tot,"sla_pct":sla,"mttr":mttr_v,"p1":p1_v,
                    "ftf_pct":ftf_pct,"aged_ct":aging_ct}

        cur_df  = df[df[_mom_col]==cur_m]
        prev_df = df[df[_mom_col]==prev_m]
        lyr_df  = df[df[_mom_col]==lyr_m] if has_lyr else None

        cur_stats  = _month_stats(cur_df)
        prev_stats = _month_stats(prev_df)
        lyr_stats  = _month_stats(lyr_df) if lyr_df is not None else None

        # Per-assignee comparison between cur and prev month
        assignee_mom = []
        if F["Assignee"] and "Assignee" in cur_df.columns:
            all_assignees_2m = set(cur_df["Assignee"].dropna()) | set(prev_df["Assignee"].dropna())
            for aname in sorted(all_assignees_2m):
                a_cur  = cur_df[cur_df["Assignee"]==aname]
                a_prev = prev_df[prev_df["Assignee"]==aname]
                grp = cur_df[cur_df["Assignee"]==aname]["AssignedGroup"].mode()
                grp = grp.iloc[0] if len(grp) else (prev_df[prev_df["Assignee"]==aname]["AssignedGroup"].mode().iloc[0] if len(prev_df[prev_df["Assignee"]==aname]["AssignedGroup"].mode()) else "—")
                assignee_mom.append({
                    "Assignee":      aname,
                    "AssignedGroup": grp,
                    "Cur_Total":     len(a_cur),
                    "Prev_Total":    len(a_prev),
                    "Delta":         len(a_cur) - len(a_prev),
                })
            assignee_mom.sort(key=lambda x: x["Cur_Total"], reverse=True)

        mom_data = {
            "cur_month": cur_m, "prev_month": prev_m,
            "mom_date_basis": "LastResolvedDate" if _mom_col == "Month_Resolved" else (date_ref or "Month"),
            "lyr_month": lyr_m if has_lyr else None,
            "cur":  cur_stats,  "prev": prev_stats,
            "lyr":  lyr_stats,
            "assignee_mom": assignee_mom,
        }
...
# MTTR by Priority
mttr_pri_labels = mttr_pri_vals = []
if F["Priority"] and df["MTTR_Hours"].notna().any():
    mp = df.groupby("Priority")["MTTR_Hours"].mean().round(1).reset_index()
    mp.columns = ["Priority","mttr"]
    mp["_sort"] = mp["Priority"].apply(priority_sort_key)
    mp = mp.sort_values("_sort").drop("_sort", axis=1)
    mttr_pri_labels = mp["Priority"].tolist()
    mttr_pri_vals   = mp["mttr"].tolist()
...
# Uses df_tag (ALL rows including duplicates) — intentional, every tag row counts
...
multi_tag_grp_rows = []   # Incidents tagged under 2+ tag types
...
if kb_available:
    type_col = df_tag["Request_Type01"].str.strip().str.lower().fillna("")
    df_tag["_kb_id"]   = df_tag["Request_Desc01"].apply(extract_kb)
    df_tag["_kb_desc"] = df_tag["Request_Desc01"].apply(extract_kb_desc)
    ...
    rkm_df   = df_tag[mask_rkm]
    ke_df    = df_tag[mask_ke]
    ps_df    = df_tag[mask_ps]
    none_df  = df_tag[~mask_any]   # truly untagged (includes blanks + other values)
    ...
    if F["AssignedGroup"]:
        for gname, gdf in df_tag.groupby("AssignedGroup"):
    ...
    untag_detail_cols = [inc_col, "Priority", "Service_Type", "Status", "Assignee"]
    untagged_grp_rows = make_incident_by_group(none_df, untag_detail_cols, "Untagged")

    # No-KB incidents (RKM rows with no KB tag)
    nokb_detail_cols = [inc_col, "Service_Type", "Status", "Assignee"]
    nokb_grp_rows    = make_incident_by_group(none_df, nokb_detail_cols, "No KB")

    # ── Multi-tagged incidents ─────────────────────────────────────────────
    # Find incidents (by Incident_Number) that appear under 2+ different tag types.
    # Uses df_tag (all rows). If no Incident_Number column, skip.
    multi_tag_grp_rows = []
    if inc_col in df_tag.columns:
        # Build a mapping: incident_id → set of tag types it carries
        tag_label_map = {}
        for mask, label in [(mask_rkm, "RKM Solution"),
                             (mask_ke,  "Known Error"),
                             (mask_ps,  "Problem Investigation")]:
            for iid in df_tag.loc[mask, inc_col].dropna().unique():
                tag_label_map.setdefault(str(iid), set()).add(label)

        # Keep only incidents with 2+ tag types
        multi_ids = {iid for iid, tags in tag_label_map.items() if len(tags) >= 2}

        if multi_ids:
            # Pull ONE representative row per incident from df_tag
            # (preferring the RKM row for context, but any row gives us the metadata)
            multi_df = (
                df_tag[df_tag[inc_col].astype(str).isin(multi_ids)]
                .drop_duplicates(subset=[inc_col], keep="first")
                .copy()
            )
            # Add a "Tags" column listing all tag types for that incident
            multi_df["Tags"] = (
                multi_df[inc_col].astype(str)
                .map(lambda iid: " + ".join(sorted(tag_label_map.get(iid, set()))))
            )
            multi_detail_cols = [
                c for c in [inc_col, "Tags", "Assignee", "Service_Type",
                             "HPD_CI", "Priority", "AssignedGroup"]
                if c in multi_df.columns
            ]
            multi_tag_grp_rows = make_incident_by_group(
                multi_df, multi_detail_cols, "Multi-Tagged"
            )
            log.info("Multi-tagged incidents: %d unique IDs across %d groups",
                     len(multi_ids), len(multi_tag_grp_rows))
...
def _gen_all_charts(theme_name):
    ...
    c["sla_donut"] = make_donut(sla_labels, sla_data,
                                colors=[sla_color(l) for l in sla_labels],
                                title="SLA Status")
    ...
    if mttr_pri_labels:
        c["mttr_pri"] = make_hbar(
            mttr_pri_labels, mttr_pri_vals,
            color=[PRI_COLORS.get(l, YELLOW) for l in mttr_pri_labels],
            title="Avg MTTR by Priority (hrs)", xlabel="Hours"
        )
...
return {
    ...
    "mttr_pri_labels": mttr_pri_labels, "mttr_pri_vals": mttr_pri_vals,
    ...
    "multi_tag_grp_rows": multi_tag_grp_rows,   # Multi-tagged incidents — grouped
    ...
    "raw_row_count":      len(df_tag),
    "unique_inc_count":   len(df),
}
```
...
<div id="dash">
  ...
  <nav class="tabs">
    <div class="tab on"  onclick="go('ex',this)">🎯 Executive</div>
    <div class="tab"     onclick="go('ov',this)">📊 Overview</div>
  ...
  <section class="page" id="pg-ex">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-size:1.1rem;font-weight:800;letter-spacing:-.02em">Executive Summary</div>
        <div style="font-size:.75rem;color:var(--muted);margin-top:2px" id="ex-date-range">Upload a file to view</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
        <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:8px 16px;text-align:center">
          <div style="font-size:.62rem;color:var(--muted);text-transform:uppercase;letter-spacing:.08em">Health Score</div>
          <div id="ex-health-val" style="font-size:1.8rem;font-weight:800;font-family:monospace;color:var(--green)">—</div>
          <div id="ex-health-lbl" style="font-size:.65rem;color:var(--muted)">—</div>
        </div>
      </div>
    </div>
...
<div class="g2" style="margin-top:14px">
  <div class="cc">
    <div class="cc-hd"><span class="cc-dot" style="background:var(--red)"></span><span class="cc-title">🚨 Top Issues Requiring Attention</span></div>
    <div id="ex-issues" style="font-size:.82rem;line-height:2"></div>
  </div>
  <div class="cc">
    <div class="cc-hd"><span class="cc-dot" style="background:var(--green)"></span><span class="cc-title">✅ Positive Signals</span></div>
    <div id="ex-positives" style="font-size:.82rem;line-height:2"></div>
  </div>
</div>

<!-- Performance scoreboard -->
<div class="sec" style="margin-top:18px">📊 Group Performance Scoreboard</div>
<div class="full cc">
  <div class="cc-hd"><span class="cc-dot" style="background:var(--blue)"></span>
    <span class="cc-title">AssignedGroup — Ranked by Overall Performance</span>
    <span style="font-size:.68rem;color:var(--dim);margin-left:8px">Scored on SLA% + FTF% + MTTR</span>
  </div>
  <div class="tbl-wrap"><table><thead><tr>
    <th>Rank</th><th>AssignedGroup</th><th>Total</th><th>SLA %</th>
    <th>Avg MTTR</th><th>FTF %</th><th>Avg Hops</th><th>Score</th><th>Status</th>
  </tr></thead><tbody id="ex-grp-tbody"></tbody></table></div>
</div>

<!-- P1 spotlight -->
<div class="g2" style="margin-top:14px">
  <div class="cc">
    <div class="cc-hd"><span class="cc-dot" style="background:var(--red)"></span><span class="cc-title">🔴 P1 Critical — SLA Breaches</span></div>
    <div id="ex-p1-count" style="font-size:.82rem;color:var(--muted)"></div>
    <div class="tbl-wrap" style="margin-top:8px"><table><thead><tr><th>Incident</th><th>Group</th><th>MTTR</th></tr></thead>
    <tbody id="ex-p1-tbody"></tbody></table></div>
  </div>
  <div class="cc">
    <div class="cc-hd"><span class="cc-dot" style="background:var(--orange)"></span><span class="cc-title">🔁 Top Repeat-Offender Assets (≥5 incidents)</span></div>
    <div class="tbl-wrap"><table><thead><tr><th>HPD_CI</th><th>Incidents</th><th>Volume</th></tr></thead>
    <tbody id="ex-ci-tbody"></tbody></table></div>
  </div>
</div>
```
...
<!-- Month-over-Month Comparison -->
<div class="sec" id="mom-sec" style="display:none">📅 Month-over-Month Comparison
  <span id="mom-title" style="font-size:.72rem;font-weight:400;color:var(--blue);margin-left:8px"></span>
</div>

<div id="mom-section" style="display:none">
  <!-- KPI comparison table -->
  <div class="full cc" style="margin-bottom:14px">
    <div class="cc-hd"><span class="cc-dot" style="background:var(--blue)"></span>
      <span class="cc-title">Key Metrics — Current vs Previous Month vs Same Month Last Year</span>
    </div>
    <div class="tbl-wrap"><table><thead><tr>
      <th>Metric</th>
      <th id="mom-th-cur">Current</th>
      <th id="mom-th-prev">Previous</th><th>Δ MoM</th>
      <th id="mom-th-lyr" style="display:none">Last Year</th>
      <th id="mom-th-yoy" style="display:none">Δ YoY</th>
    </tr></thead><tbody id="mom-tbody"></tbody></table></div>
  </div>

  <!-- Assignee MoM comparison -->
  <div class="full al-card">
    <div class="al-hdr" style="border-left:3px solid var(--purple)" onclick="toggleAl(this,'assignee-mom-body')">
      <span style="font-size:1rem">👤</span>
      <div class="al-info">
        <div class="al-title">Assignee Performance — Current Month vs Previous Month</div>
        <div class="al-sub">Who was the top performer last month? Who is leading this month?</div>
      </div>
      <span class="al-expand">▼</span>
    </div>
    <div class="al-body" id="assignee-mom-body">
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search assignee or group…" oninput="filterTbl(this,'amom-tbody')"/>
        <button class="export-btn" onclick="exportCSV('amom-tbody','assignee_mom')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead><tr>
        <th>Rank</th><th>Assignee</th><th>Group</th>
        <th id="amom-th-cur">Current</th><th id="amom-th-prev">Previous</th>
        <th>Δ Incidents</th><th>Trend</th>
      </tr></thead><tbody id="amom-tbody"></tbody></table></div>
    </div>
  </div>
</div>

<div id="mom-na" style="display:none;padding:14px;color:var(--dim);font-size:.8rem;
  background:var(--card);border:1px solid var(--border);border-radius:10px;margin-top:8px">
  ℹ Month-over-Month comparison requires at least 2 months of data.
</div>
```
  ...
  <section class="page" id="pg-sl">
    ...
    <div class="g2">
      <div class="cc"><div class="cc-hd"><span class="cc-dot" style="background:var(--red)"></span>
        <span class="cc-title">Avg MTTR by Priority (hrs)</span>
        <span class="cc-badge">P1 critical gets resolved fastest?</span></div>
        <div id="c-mttr-pri"></div>
      </div>
      <div class="cc" id="svc-sla-card" style="display:none"><div class="cc-hd"><span class="cc-dot" style="background:var(--green)"></span>
        <span class="cc-title">SLA Compliance % by Service Type</span></div>
        <div id="c-svc-sla-2"></div>
      </div>
    </div>
...
<div class="sec">🏆 Resolver Leaderboard — All Assignees Ranked</div>
<div class="full cc">
  <div class="cc-hd"><span class="cc-dot" style="background:var(--yellow)"></span>
    <span class="cc-title">All Resolvers — Ranked by Incident Volume · Tier-coloured</span>
    <span style="font-size:.68rem;color:var(--dim);margin-left:8px">
      🏆 Top 25% &nbsp;✅ 50–75% &nbsp;⚠ 25–50% &nbsp;🔴 Bottom 25%
    </span>
  </div>
  <div class="tbl-toolbar">
    <input class="search-box" placeholder="Search assignee or group…" oninput="filterTbl(this,'lb-tbody')"/>
    <button class="export-btn" onclick="exportCSV('lb-tbody','resolver_leaderboard')">⬇ CSV</button>
  </div>
  <div class="tbl-wrap"><table><thead><tr>
    <th>Rank</th><th>Assignee</th><th>Group</th><th>Total</th><th>Resolved</th>
    <th>SLA %</th><th>Avg MTTR</th><th>FTF</th><th>FTF %</th><th>Avg Hops</th><th>Tier</th>
  </tr></thead><tbody id="lb-tbody"></tbody></table></div>
</div>

<!-- Workload Distribution -->
<div class="sec">📦 Workload Distribution by Priority — per Group</div>
<div id="workload-panels"></div>

<!-- Per-Group Performance (existing) -->
<div class="sec">👥 Per-Group Detail — click header to expand assignees</div>
...
```
<!-- Dedup info banner — shown only when duplicate rows exist -->
<div id="dedup-banner" style="display:none;margin-bottom:14px;padding:11px 16px;
  background:rgba(74,140,255,.08);border:1px solid rgba(74,140,255,.25);
  border-radius:10px;font-size:.78rem;line-height:1.6">
  <span style="font-weight:700;color:var(--blue)">ℹ Tagging Data Note</span>
  <span id="dedup-text" style="color:var(--muted);margin-left:8px"></span>
</div>
...
<!-- Multi-Tagged Incidents — grouped by AssignedGroup -->
<div class="full al-card">
  <div class="al-hdr" style="border-left:3px solid var(--yellow)" onclick="toggleAl(this,'multi-tag-body')">
    <span style="font-size:1rem">🔀</span>
    <div class="al-info">
      <div class="al-title">Multi-Tagged Incidents — Same Incident Tagged Under 2 or More Types</div>
      <div class="al-sub">Grouped by AssignedGroup · expand to see Incident, Tags, Assignee, Service Type, HPD CI, Priority</div>
    </div>
    <span class="al-badge" id="multi-tag-badge" style="background:rgba(255,194,64,.15);color:var(--yellow)">0</span>
    <span class="al-expand">▼</span>
  </div>
  <div class="al-body" id="multi-tag-body">
    <div id="multi-tag-panels"></div>
  </div>
</div>
...
// SLA status colour — mirrors Python sla_color() logic
function slaChipColor(v){
  const l=(v||'').toLowerCase().trim();
  if(/met|within|compliant|^ok$|^yes$|pass/.test(l)) return '#30d988';   // GREEN
  if(/breach|miss|violat|fail|overdue|^no$/.test(l))  return '#ff4f6a';   // RED
  if(/pend|progress|^open$/.test(l))                   return '#ffc240';   // YELLOW
  if(/invalid|n\/a|cancel|not app/.test(l))             return '#fb923c';   // ORANGE
  if(/exempt|exclud|skip/.test(l))                      return '#94a3b8';   // SLATE
  return '#4a8cff';                                                          // BLUE (unknown)
}
...
function build(d){
  ...
  buildExecutive(d); buildOrgTable(d); buildTeamPanels(d);
  buildLeaderboard(d); buildWorkload(d); buildMoM(d); buildCI(d);
...
function grpAlertHTML(grpList){
  ...
  return grpList.map(g=>{
    ...
    const tbody=g.rows.map(r=>'<tr>'+cols.map(c=>{
      ...
      if(c==='SLAStatus'){
        const sc=slaChipColor(v);
        return `<td><span class="chip" style="background:${sc}22;color:${sc}">${v}</span></td>`;}
      return `<td>${v}</td>`;
    }).join('')+'</tr>').join('');
...
function buildCharts(d){
  ...
  if(d.charts.mttr_pri) setImg('c-mttr-pri', d.charts.mttr_pri, (d.charts_light||{}).mttr_pri);
  // ci
...
function buildKB(d){
  ...
  // ── Dedup banner ─────────────────────────────────────────────────────────────
  if(d.dedup_note && d.dedup_note.length){
    const banner=$('dedup-banner'), txt=$('dedup-text');
    if(banner) banner.style.display='';
    if(txt)    txt.textContent = d.dedup_note;
  }

  // ── KPIs ────────────────────────────────────────────────────────────────────
  $('kb-kpis').innerHTML=[
    {l:'RKM Solution',          v:s.total_rkm||0,     c:'var(--purple)', i:'📚', sub:'KB-tagged incidents'},
    {l:'Known Error',           v:s.total_ke||0,       c:'var(--orange)', i:'🔶', sub:'Known Error tag rows'},
    {l:'Problem Investigation', v:s.total_ps||0,       c:'var(--cyan)',   i:'🔷', sub:'Problem Investigation tag rows'},
    {l:'Untagged',              v:s.total_untagged||0, c:'var(--red)',    i:'🚫', sub:'No recognised tag rows'},
  ...
  renderIncidentGrpPanels('untag-panels','untag-badge',
    ...
    [
      ...
      {key:'Service_Type',    label:'Service Type', style:'font-size:.73rem;color:var(--muted)'},
  ...
  // ── No-KB incidents — grouped panels ────────────────────────────────────────
  renderIncidentGrpPanels('nokb-panels','nokb-badge',
    d.nokb_grp_rows||[],
    [
      {key:'Incident_Number', label:'Incident #',   style:'font-family:monospace;font-weight:700;font-size:.75rem;color:var(--cyan)'},
      {key:'Service_Type',    label:'Service Type', style:'font-size:.73rem;color:var(--muted)'},
      {key:'Status',          label:'Status',       style:'font-size:.74rem'},
      {key:'Assignee',        label:'Assignee',     style:'font-size:.74rem'},
    ],
    'var(--red)', 'no_kb');

  // ── Multi-Tagged Incidents ──────────────────────────────────────────────────
  renderIncidentGrpPanels('multi-tag-panels','multi-tag-badge',
    d.multi_tag_grp_rows||[],
    [
      {key:'Incident_Number', label:'Incident #',   style:'font-family:monospace;font-weight:700;font-size:.75rem;color:var(--yellow)'},
      {key:'Tags',            label:'Tagged As',    style:'font-size:.73rem;color:var(--purple);font-weight:600'},
      {key:'Assignee',        label:'Assignee',     style:'font-size:.74rem'},
      {key:'Service_Type',    label:'Service Type', style:'font-size:.73rem;color:var(--muted)'},
      {key:'HPD_CI',          label:'HPD CI',       style:'font-family:monospace;font-size:.74rem;color:var(--cyan)'},
      {key:'Priority',        label:'Priority',     style:'font-size:.73rem'},
    ],
    'var(--yellow)', 'multi_tagged');
...
function buildExecutive(d){
  // Date range
  if($('ex-date-range')) $('ex-date-range').textContent=`Data: ${d.date_min} → ${d.date_max} · ${d.total} unique incidents`;

  // Health score
  const h=d.health||0;
  const hc=h>=70?'var(--green)':h>=50?'var(--yellow)':'var(--red)';
  const hl=h>=70?'Healthy':h>=50?'Needs Attention':'Critical';
  if($('ex-health-val')){$('ex-health-val').textContent=h+'/100';$('ex-health-val').style.color=hc;}
  if($('ex-health-lbl')){$('ex-health-lbl').textContent=hl;$('ex-health-lbl').style.color=hc;}

  // KPI cards
  const sla=d.sla_pct||0, slac=sla>=70?'var(--green)':sla>=50?'var(--yellow)':'var(--red)';
  const mc=d.mttr<=24?'var(--green)':d.mttr<=72?'var(--yellow)':'var(--red)';
  const kpis=[
    {l:'Total Incidents',    v:d.total,         c:'var(--blue)',  i:'📋', s:`${d.date_min} → ${d.date_max}`},
    {l:'SLA Compliance',     v:sla+'%',          c:slac,           i:'🎯', s:`${d.sla_met_ct} incidents met SLA`},
    {l:'Avg MTTR',           v:d.mttr+'h',       c:mc,             i:'⏱', s:'Mean time to resolve'},
    {l:'P1 Critical',        v:d.p1_ct,          c:'var(--red)',   i:'🚨', s:'Critical priority count'},
    {l:'MTTR Alerts (>200h)',v:d.mttr_alert_total,c:'var(--orange)',i:'⚠', s:'Extreme resolution time'},
    {l:'HOP Alerts (≥5)',    v:d.hop_alert_total, c:'var(--purple)',i:'🔄', s:'High group-transfer count'},
  ];
  if($('ex-kpis')) $('ex-kpis').innerHTML=kpis.map(k=>`<div class="kpi">
    <div class="kpi-bar" style="background:${k.c}"></div><div class="kpi-icon">${k.i}</div>
    <div class="kpi-lbl">${k.l}</div>
    <div class="kpi-val" style="color:${k.c}">${k.v}</div>
    <div class="kpi-sub">${k.s}</div></div>`).join('');
  ...
  if(sla<70) issues.push(`⚠ SLA compliance is <strong>${sla}%</strong> — below 70% target`);
  if(d.mttr>24) issues.push(`⏱ Avg MTTR is <strong>${d.mttr}h</strong> — exceeds 24h target`);
  if(d.mttr_alert_total>0) issues.push(`🔴 <strong>${d.mttr_alert_total}</strong> incidents took &gt;200h to resolve`);
  if(d.hop_alert_total>0) issues.push(`🔄 <strong>${d.hop_alert_total}</strong> incidents bounced across ≥5 groups`);
  if(d.p1_breach_rows&&d.p1_breach_rows.length>0) issues.push(`🚨 <strong>${d.p1_breach_rows.length}</strong> P1 Critical incidents breached SLA`);
  if(issues.length===0) issues.push('✅ No critical issues detected');
  if(sla>=70) positives.push(`✅ SLA compliance <strong>${sla}%</strong> — meeting target`);
  if(d.mttr<=24) positives.push(`✅ Avg MTTR <strong>${d.mttr}h</strong> — within 24h target`);
  const ftfTop=(d.ftf_rows||[]).filter(r=>r.ftf_pct>=80);
  if(ftfTop.length) positives.push(`🏆 <strong>${ftfTop.length}</strong> groups with FTF ≥ 80%`);
  if(positives.length===0) positives.push('📋 No positive signals to highlight yet');
  if($('ex-issues')) $('ex-issues').innerHTML=issues.map(s=>`<div style="padding:4px 0;border-bottom:1px solid var(--border)">${s}</div>`).join('');
  if($('ex-positives')) $('ex-positives').innerHTML=positives.map(s=>`<div style="padding:4px 0;border-bottom:1px solid var(--border)">${s}</div>`).join('');

  // Group scoreboard — score = SLA%×0.4 + FTF%×0.3 + (100-clamp(MTTR,0,200)/2)×0.3
  const ftfMap={};(d.ftf_rows||[]).forEach(r=>ftfMap[r.group]={ftf_pct:r.ftf_pct,ftf:r.ftf});
  const grpRows=(d.group_tables||[]).map(g=>{
    const sla_s=typeof g.sla_pct==='number'?g.sla_pct:0;
    const ftf_s=(ftfMap[g.group]||{}).ftf_pct||0;
    const mttr_n=typeof g.mttr==='number'?g.mttr:100;
    const score=Math.round(sla_s*0.4+ftf_s*0.3+(100-Math.min(mttr_n/2,100))*0.3);
    return {...g, ftf_pct:ftf_s, score};
  }).sort((a,b)=>b.score-a.score);
  const maxScore=grpRows.length?Math.max(...grpRows.map(r=>r.score)):100;
  if($('ex-grp-tbody')) $('ex-grp-tbody').innerHTML=grpRows.map((g,i)=>{
    const slac2=typeof g.sla_pct==='number'?(g.sla_pct>=70?'c-met':g.sla_pct>=50?'c-pend':'c-breach'):'';
    const sc_pct=Math.round(g.score/maxScore*100);
    const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':'';
    const status=g.score>=70?'🟢 Healthy':g.score>=50?'🟡 Moderate':'🔴 Needs Focus';
    return `<tr>
      <td style="font-family:monospace;font-weight:700">${medal||'#'+(i+1)}</td>
      <td style="font-weight:700">${g.group}</td>
      <td style="font-family:monospace">${g.total}</td>
      <td><span class="chip ${slac2}">${typeof g.sla_pct==='number'?g.sla_pct+'%':'—'}</span></td>
      <td style="font-family:monospace">${g.mttr}h</td>
      <td style="font-family:monospace">${g.ftf_pct}%</td>
      <td style="font-family:monospace">${g.avg_transfers}</td>
      <td><div class="mbar"><div class="mbar-bg"><div class="mbar-fill" style="width:${sc_pct}%;background:var(--blue)"></div></div>
          <span style="font-family:monospace;font-size:.72rem">${g.score}</span></div></td>
      <td>${status}</td>
    </tr>`;
  }).join('');

  // P1 breaches spotlight (top 5)
  const p1b=d.p1_breach_rows||[];
  if($('ex-p1-count')) $('ex-p1-count').textContent=`${p1b.length} P1 incidents breached SLA`;
  const p1cols=['Incident_Number','AssignedGroup','MTTR_Hours'];
  if($('ex-p1-tbody')) $('ex-p1-tbody').innerHTML=(p1b.slice(0,5)).map(r=>`<tr>
    <td style="font-family:monospace;font-size:.76rem;color:var(--cyan)">${r.Incident_Number||r[Object.keys(r)[0]]||'—'}</td>
    <td style="font-size:.75rem">${r.AssignedGroup||'—'}</td>
    <td style="font-family:monospace;color:var(--red)">${r.MTTR_Hours||'—'}h</td>
  </tr>`).join('')||'<tr><td colspan="3" style="color:var(--green);padding:10px">No P1 SLA breaches ✓</td></tr>';

  // Repeat offender CIs
  const rep=d.repeat_ci_rows||[];
  const maxR=rep.length?Math.max(...rep.map(r=>r.count)):1;
  if($('ex-ci-tbody')) $('ex-ci-tbody').innerHTML=rep.slice(0,8).map(r=>{
    const pct=Math.round(r.count/maxR*100);
    return `<tr>
      <td style="font-weight:700;color:var(--orange);font-family:monospace">${r.HPD_CI}</td>
      <td><span class="chip c-orange">${r.count}</span></td>
      <td><div class="mbar-bg" style="width:80px"><div class="mbar-fill" style="width:${pct}%;background:var(--orange)"></div></div></td>
    </tr>`;
  }).join('')||'<tr><td colspan="3" style="color:var(--green);padding:10px">No CIs with ≥5 incidents ✓</td></tr>';
}

// ── RESOLVER LEADERBOARD ──────────────────────────────────────────────────────
function buildLeaderboard(d){
  const rows=d.resolver_rows||[];
  if(!rows.length){
    if($('lb-tbody')) $('lb-tbody').innerHTML='<tr><td colspan="11" style="color:var(--dim);padding:14px">No Assignee data found</td></tr>';
    return;
  }
  const tierColor={'🏆 Top':'var(--green)','✅ Good':'var(--blue)','⚠ Mid':'var(--yellow)','🔴 Low':'var(--red)'};
  if($('lb-tbody')) $('lb-tbody').innerHTML=rows.map(r=>{
    const slac=typeof r.SLA_Pct==='number'?(r.SLA_Pct>=70?'c-met':r.SLA_Pct>=50?'c-pend':'c-breach'):'';
    const tc=tierColor[r.Tier]||'var(--muted)';
    const medal=r.Rank===1?'🥇':r.Rank===2?'🥈':r.Rank===3?'🥉':'#'+r.Rank;
    return `<tr>
      <td style="font-family:monospace;font-weight:700">${medal}</td>
      <td style="font-weight:700">${r.Assignee}</td>
      <td style="font-size:.74rem;color:var(--muted)">${r.AssignedGroup}</td>
      <td style="font-family:monospace;font-weight:700">${r.Total}</td>
      <td style="font-family:monospace">${r.Resolved}</td>
      <td>${typeof r.SLA_Pct==='number'?`<span class="chip ${slac}">${r.SLA_Pct}%</span>`:'—'}</td>
      <td style="font-family:monospace">${r.Avg_MTTR!==null?r.Avg_MTTR+'h':'—'}</td>
      <td style="font-family:monospace">${r.FTF}</td>
      <td style="font-family:monospace">${r.FTF_Pct}%</td>
      <td style="font-family:monospace">${r.Avg_Hops!==null?r.Avg_Hops:'—'}</td>
      <td><span style="font-weight:700;color:${tc}">${r.Tier}</span></td>
    </tr>`;
  }).join('');
}

// ── WORKLOAD DISTRIBUTION ─────────────────────────────────────────────────────
function buildWorkload(d){
  const container=$('workload-panels');
  if(!container) return;
  const groups=d.workload_by_group||[];
  if(!groups.length){container.innerHTML='<div class="cc" style="padding:14px;color:var(--dim)">No Priority/Assignee data available</div>';return;}
  const PRI_COLS=['P1','P2','P3','P4'];
  const PRI_COLORS={'P1':'var(--red)','P2':'var(--yellow)','P3':'var(--blue)','P4':'var(--green)'};
  container.innerHTML=groups.map((g,gi)=>{
    const eid='wl'+gi+'_'+Math.random().toString(36).slice(2,6);
    const maxT=g.rows.length?Math.max(...g.rows.map(r=>r.Total)):1;
    const trs=g.rows.map(r=>{
      const bars=PRI_COLS.map(p=>{
        const v=r[p]||0, pct=Math.round(v/maxT*100);
        return v>0?`<div title="${p}: ${v}" style="height:14px;width:${pct}%;min-width:${v>0?2:0}px;background:${PRI_COLORS[p]};display:inline-block;border-right:1px solid var(--bg)"></div>`:'';
      }).join('');
      return `<tr>
        <td style="font-weight:600">${r.Assignee}</td>
        <td style="font-family:monospace;font-weight:700">${r.Total}</td>
        <td><span style="background:rgba(255,79,106,.2);color:var(--red);padding:1px 6px;border-radius:4px;font-size:.7rem">${r.P1}</span></td>
        <td><span style="background:rgba(255,194,64,.2);color:var(--yellow);padding:1px 6px;border-radius:4px;font-size:.7rem">${r.P2}</span></td>
        <td><span style="background:rgba(74,140,255,.2);color:var(--blue);padding:1px 6px;border-radius:4px;font-size:.7rem">${r.P3}</span></td>
        <td><span style="background:rgba(48,217,136,.2);color:var(--green);padding:1px 6px;border-radius:4px;font-size:.7rem">${r.P4}</span></td>
        <td style="min-width:120px"><div style="height:14px;border-radius:3px;overflow:hidden;display:flex">${bars}</div></td>
      </tr>`;
    }).join('');
    return `<div class="grp-panel" style="margin-bottom:12px">
      <div class="grp-hdr" onclick="toggleGrp(this)">
        <span>📦</span><span class="gname">${g.group}</span>
        <div class="grp-stats">
          <span class="gs">Assignees <strong>${g.rows.length}</strong></span>
          <span class="gs">Total <strong>${g.rows.reduce((s,r)=>s+r.Total,0)}</strong></span>
        </div>
        <div style="display:flex;gap:4px;margin-left:auto;margin-right:10px">
          ${PRI_COLS.map(p=>`<span style="background:${PRI_COLORS[p]};color:var(--bg);padding:1px 7px;border-radius:10px;font-size:.62rem;font-weight:700">${p}</span>`).join('')}
        </div>
        <span class="g-expand">▼</span>
      </div>
      <div class="grp-body">
        <div class="tbl-wrap"><table>
          <thead><tr><th>Assignee</th><th>Total</th><th style="color:var(--red)">P1</th><th style="color:var(--yellow)">P2</th><th style="color:var(--blue)">P3</th><th style="color:var(--green)">P4</th><th>Breakdown</th></tr></thead>
          <tbody id="${eid}">${trs}</tbody>
        </table></div>
      </div>
    </div>`;
...
function buildMoM(d){
  ...
  if(!mom.cur_month){
    if($('mom-na')) $('mom-na').style.display='';
    return;
  }
  if($('mom-sec'))     $('mom-sec').style.display='';
  if($('mom-section')) $('mom-section').style.display='';
  if($('mom-na'))      $('mom-na').style.display='none';
  if($('mom-title'))   $('mom-title').textContent=`${mom.prev_month} → ${mom.cur_month}${mom.lyr_month?' vs '+mom.lyr_month:''}`;
  // Show which date is used as grouping basis
  const basisEl=$('mom-sec');
  if(basisEl && mom.mom_date_basis){
    const chip=`<span style="font-size:.65rem;font-weight:600;background:rgba(74,140,255,.12);
      color:var(--blue);border:1px solid rgba(74,140,255,.3);padding:1px 8px;border-radius:10px;
      margin-left:8px">grouped by ${mom.mom_date_basis}</span>`;
    const existing=basisEl.querySelector('.mom-basis-chip');
    if(!existing) basisEl.insertAdjacentHTML('beforeend', chip.replace('margin-left','class="mom-basis-chip" style="margin-left'));
  }

  // Column headers
  const hasLyr=!!mom.lyr_month;
  if($('mom-th-cur'))  $('mom-th-cur').textContent=mom.cur_month;
  if($('mom-th-prev')) $('mom-th-prev').textContent=mom.prev_month;
  if($('mom-th-lyr'))  {$('mom-th-lyr').textContent=mom.lyr_month||''; $('mom-th-lyr').style.display=hasLyr?'':'none';}
  if($('mom-th-yoy'))  $('mom-th-yoy').style.display=hasLyr?'':'none';
  if($('amom-th-cur')) $('amom-th-cur').textContent=mom.cur_month;
  if($('amom-th-prev'))$('amom-th-prev').textContent=mom.prev_month;

  function delta(cur,prev,higherIsBetter=true){
    if(cur===null||prev===null) return '—';
    const d2=cur-prev, sign=d2>0?'+':'';
    const good=(higherIsBetter&&d2>0)||(!higherIsBetter&&d2<0);
    const col=good?'var(--green)':d2===0?'var(--muted)':'var(--red)';
    const arrow=d2>0?'↑':d2<0?'↓':'→';
    return `<span style="color:${col};font-weight:700">${arrow} ${sign}${typeof cur==='number'&&cur%1!==0?d2.toFixed(1):d2}</span>`;
  }
  function lyrdelta(cur,lyr,higherIsBetter=true){
    if(!hasLyr||cur===null||lyr===null) return '';
    return delta(cur,lyr,higherIsBetter);
  }

  const cur=mom.cur||{}, prev=mom.prev||{}, lyr=mom.lyr||{};
  const metrics=[
    {label:'Total Incidents',   ck:'total',   pk:'total',   lk:'total',   hib:false, fmt:v=>v},
    {label:'SLA Compliance %',  ck:'sla_pct', pk:'sla_pct', lk:'sla_pct', hib:true,  fmt:v=>v!==null?v+'%':'—'},
    {label:'Avg MTTR (hrs)',    ck:'mttr',    pk:'mttr',    lk:'mttr',    hib:false, fmt:v=>v!==null?v+'h':'—'},
    {label:'P1 Critical Count', ck:'p1',      pk:'p1',      lk:'p1',      hib:false, fmt:v=>v},
    {label:'FTF Rate %',        ck:'ftf_pct', pk:'ftf_pct', lk:'ftf_pct', hib:true,  fmt:v=>v!==null?v+'%':'—'},
    {label:'Aged Incidents (>30d)', ck:'aged_ct', pk:'aged_ct', lk:'aged_ct', hib:false, fmt:v=>v!==null&&v!==undefined?String(v):'0', note:'Incidents that took more than 30 days to resolve (AgeDays > 30)'},
  ];
  if($('mom-tbody')) $('mom-tbody').innerHTML=metrics.map(m=>{
    const cv=cur[m.ck]??null, pv=prev[m.pk]??null, lv=lyr[m.lk]??null;
    const d_mom=delta(cv,pv,m.hib), d_yoy=lyrdelta(cv,lv,m.hib);
    const noteHtml=m.note?`<div style="font-size:.65rem;color:var(--dim);margin-top:1px">${m.note}</div>`:'';
    return `<tr>
      <td style="font-weight:600">${m.label}${noteHtml}</td>
      <td style="font-family:monospace;font-weight:700">${m.fmt(cv)}</td>
      <td style="font-family:monospace;color:var(--muted)">${m.fmt(pv)}</td>
      <td>${d_mom}</td>
      <td style="${hasLyr?'':'display:none'};font-family:monospace;color:var(--muted)">${m.fmt(lv)}</td>
      <td style="${hasLyr?'':'display:none'}">${d_yoy}</td>
    </tr>`;

  // Assignee MoM
  const amom=mom.assignee_mom||[];
  const ranked=[...amom].sort((a,b)=>b.Cur_Total-a.Cur_Total);
  if($('amom-tbody')) $('amom-tbody').innerHTML=ranked.map((r,i)=>{
    const d2=r.Delta, col=d2>0?'var(--green)':d2<0?'var(--red)':'var(--muted)';
    const arrow=d2>0?'↑':d2<0?'↓':'→';
    const trend=d2>0?`<span style="color:var(--green)">↑ +${d2}</span>`:d2<0?`<span style="color:var(--red)">↓ ${d2}</span>`:`<span style="color:var(--muted)">→ 0</span>`;
    const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':'#'+(i+1);
    return `<tr>
      <td style="font-family:monospace;font-weight:700">${medal}</td>
      <td style="font-weight:600">${r.Assignee}</td>
      <td style="font-size:.73rem;color:var(--muted)">${r.AssignedGroup}</td>
      <td style="font-family:monospace;font-weight:700;color:var(--blue)">${r.Cur_Total}</td>
      <td style="font-family:monospace;color:var(--muted)">${r.Prev_Total}</td>
      <td style="font-family:monospace;color:${col}">${d2>0?'+':''}${d2}</td>
      <td>${trend}</td>
    </tr>`;
  }).join('');
}
