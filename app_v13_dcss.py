...
def analyse(df_raw):
    ...
    svc_labels = svc_data = []
    svc_sla_labels = svc_sla_vals = []     # SLA % by Service_Type
    svc_mttr_labels = svc_mttr_vals = []   # MTTR by Service_Type
    svc_vol_labels = svc_vol_data = []     # Volume by Service_Type (for stacked monthly)
    if F["Service_Type"]:
    ...
    # Service_Type charts (shown on SLA tab)
    if svc_sla_labels:
        charts["svc_sla"] = make_hbar_colored(
            svc_sla_labels, svc_sla_vals,
            title="SLA Compliance % by Service Type", xlabel="SLA Met %"
        )
    if svc_mttr_labels:
        charts["svc_mttr"] = make_hbar(
            svc_mttr_labels, svc_mttr_vals, color=YELLOW,
            title="Avg MTTR by Service Type (hrs)", xlabel="Hours"
        )
    if svc_labels:
        charts["svc_donut"] = make_donut(
            svc_labels, svc_data,
            colors=[PALETTE[i % len(PALETTE)] for i in range(len(svc_labels))],
            title="Incident Volume by Service Type"
        )
    # Service_Type monthly volume as stacked line
    if svc_vol_months and svc_vol_series:
        svc_series_for_chart = [
            (name, vals, PALETTE[i % len(PALETTE)])
            for i, (name, vals) in enumerate(svc_vol_series)
        ]
        charts["svc_trend"] = make_stacked_bar(
            svc_vol_months, svc_series_for_chart,
...
function buildCharts(d){
  ...
  setImg('c-svc-donut', (d.charts||{}).svc_donut);
  setImg('c-svc-sla',   (d.charts||{}).svc_sla);
  setImg('c-svc-mttr',  (d.charts||{}).svc_mttr);
  setImg('c-svc-trend', (d.charts||{}).svc_trend);
