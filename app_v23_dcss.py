...
def analyse(df_raw):
    ...
    if F["AssignedGroup"] and F["Assignee"] and F["Priority"]:
        ...
        for gname, gdf in df.groupby("AssignedGroup"):
            wl_pivot = (gdf.groupby(["Assignee","Priority"]).size()
                          .unstack(fill_value=0).reset_index())
            # Ensure all 4 priority columns exist
            for p in priorities:
                if p not in wl_pivot.columns:
                    wl_pivot[p] = 0
            wl_pivot["Total"] = wl_pivot[[p for p in priorities if p in wl_pivot.columns]].sum(axis=1)
            wl_pivot = wl_pivot.sort_values("Total", ascending=False)
            rows_out = []
            for _, r in wl_pivot.iterrows():
                rows_out.append({
                    "Assignee": r["Assignee"],
                    "Total":    int(r["Total"]),
                    "P1":       int(r.get("P1 - Critical", 0)),
                    "P2":       int(r.get("P2 - High", 0)),
                    "P3":       int(r.get("P3 - Medium", 0)),
                    "P4":       int(r.get("P4 - Low", 0)),
                })
    ...
    if _mom_col in df.columns and df[_mom_col].notna().any():
        ...
        if len(months_sorted) >= 2:
            ...
            def _month_stats(mdf):
                ...
                return {"total":tot,"sla_pct":sla,"mttr":mttr_v,"p1":p1_v,"ftf_pct":ftf_pct}
