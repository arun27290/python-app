...
def analyse(df_raw):
    ...
    if kb_available:
        ...
        untag_detail_cols = [inc_col, "Priority", "Request_Type01", "Status", "Assignee", "Service_Type"]
        untagged_grp_rows = make_incident_by_group(none_df, untag_detail_cols, "Untagged")

        # No-KB incidents — v15 logic restored exactly:
        # source = none_df (incidents with no RKM/KE/PI tag at all)
        # + Service_Type column added as new requirement
...
function buildKB(d){
  ...
  // ── No-KB incidents — grouped panels (v15 logic, + Service_Type col) ─────────
  renderIncidentGrpPanels('nokb-panels','nokb-badge',
    ...
    [
      ...
      {key:'Assignee',        label:'Assignee',     style:'font-size:.74rem'},
      {key:'Service_Type',    label:'Service Type', style:'font-size:.74rem;color:var(--muted)'},
    ],
    'var(--red)', 'no_kb');
