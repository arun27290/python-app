...
def analyse(df_raw):
    ...
    kb_no_kb_rows    = []
    kb_summary_data  = {}
    kb_article_rows  = []      # RKM Solution desc table
    ke_desc_rows     = []      # Known Error desc table
    pi_desc_rows     = []      # Problem Investigation desc table
    ...
    if kb_available:
        ...
        def make_desc_table(source_df, tag_label):
            """
            Simple pivot: group source_df by Request_Desc01 (raw, unchanged).
            Returns list of dicts with keys:
              Request_Description, Incident_Count, AssignedGroup
            ...
            # Fill blanks so they don't vanish from the groupby
            work[desc_col] = work[desc_col].fillna("(blank)").astype(str).str.strip()
            work[desc_col] = work[desc_col].replace("", "(blank)")

            id_col = inc_col if inc_col in work.columns else work.columns[0]

            if F["AssignedGroup"] and "AssignedGroup" in work.columns:
                # Group by (Description, AssignedGroup) for the full breakdown
                grp = (
                    work.groupby([desc_col, "AssignedGroup"], dropna=False)
                    .size()
                    .reset_index(name="cnt")
                    .sort_values([desc_col, "cnt"], ascending=[True, False])
                )
                # Roll up to (Description) level for total count and group list
                desc_totals = (
                ...
                # Build AssignedGroup string per description
                grp_map = {}
                for _, r in grp.iterrows():
                    d = r[desc_col]
                    g = r["AssignedGroup"]
                    c = r["cnt"]
                    grp_map.setdefault(d, []).append(f"{g} ({c})")

                rows_out = []
                for _, r in desc_totals.iterrows():
                    d = r[desc_col]
                    rows_out.append({
                        "Request_Description": d,
                        "Incident_Count":      int(r["Incident_Count"]),
                        "AssignedGroup":       " | ".join(grp_map.get(d, ["—"])),
                    })
                return rows_out
            else:
                # No AssignedGroup column — just count per description
                desc_totals = (
                    work.groupby(desc_col, dropna=False)
                    .size()
                    .reset_index(name="Incident_Count")
        ...
        kb_article_rows = make_desc_table(rkm_df,  "RKM Solution")
        ke_desc_rows    = make_desc_table(ke_df,   "Known Error")
        pi_desc_rows    = make_desc_table(ps_df,   "Problem Investigation")

        log.info("Desc tables — RKM:%d  KE:%d  PI:%d",
    ...
    return {
        ...
        "kb_article_rows":  kb_article_rows,   # RKM Solution desc table
        "ke_desc_rows":     ke_desc_rows,       # Known Error desc table
        "pi_desc_rows":     pi_desc_rows,       # Problem Investigation desc table
...
<div id="dash">
  ...
  <section class="page" id="pg-kb">
    ...
    <!-- ── Three Request Description tables (one per tag type) ── -->
    <div class="sec">📋 Request Description — Incident Count by Tag Type</div>
    ...
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--purple);cursor:default">
        <span style="font-size:1rem">📚</span>
        <div class="al-info">
          <div class="al-title">RKM Solution — Request Description · Incident Count · AssignedGroup</div>
          <div class="al-sub">Grouped by raw Request Description01 value, no parsing applied</div>
        </div>
        <span class="al-badge" id="rkm-desc-badge" style="background:rgba(167,139,250,.15);color:var(--purple)">0</span>
      </div>
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search description or group…" oninput="filterTbl(this,'rkm-desc-tbody')"/>
        <button class="export-btn" onclick="exportCSV('rkm-desc-tbody','rkm_solution_descriptions')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead><tr>
        <th>#</th><th>Request Description</th><th>Incident Count</th><th>AssignedGroup</th><th>Volume</th>
      </tr></thead><tbody id="rkm-desc-tbody"></tbody></table></div>
    </div>

    <!-- Known Error table -->
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--orange);cursor:default">
        <span style="font-size:1rem">🔶</span>
        <div class="al-info">
          <div class="al-title">Known Error — Request Description · Incident Count · AssignedGroup</div>
          <div class="al-sub">Grouped by raw Request Description01 value</div>
        </div>
        <span class="al-badge" id="ke-desc-badge" style="background:rgba(251,146,60,.15);color:var(--orange)">0</span>
      </div>
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search description or group…" oninput="filterTbl(this,'ke-desc-tbody')"/>
        <button class="export-btn" onclick="exportCSV('ke-desc-tbody','known_error_descriptions')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead><tr>
        <th>#</th><th>Request Description</th><th>Incident Count</th><th>AssignedGroup</th><th>Volume</th>
      </tr></thead><tbody id="ke-desc-tbody"></tbody></table></div>
    </div>

    <!-- Problem Investigation table -->
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--cyan);cursor:default">
        <span style="font-size:1rem">🔷</span>
        <div class="al-info">
          <div class="al-title">Problem Investigation — Request Description · Incident Count · AssignedGroup</div>
          <div class="al-sub">Grouped by raw Request Description01 value</div>
        </div>
        <span class="al-badge" id="pi-desc-badge" style="background:rgba(34,211,238,.15);color:var(--cyan)">0</span>
      </div>
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search description or group…" oninput="filterTbl(this,'pi-desc-tbody')"/>
        <button class="export-btn" onclick="exportCSV('pi-desc-tbody','problem_investigation_descriptions')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead><tr>
        <th>#</th><th>Request Description</th><th>Incident Count</th><th>AssignedGroup</th><th>Volume</th>
      </tr></thead><tbody id="pi-desc-tbody"></tbody></table></div>
    </div>
...
function buildKB(d){
  ...
  // Helper: render a description table into a tbody
  function renderDescTable(tbodyId, badgeId, rows, accentColor) {
    const badge = $(badgeId);
    const tbody = $(tbodyId);
    if (!rows || !rows.length) {
      if (badge) badge.textContent = '0';
      if (tbody) tbody.innerHTML = '<tr><td colspan="5" style="color:var(--dim);padding:14px">No data for this tag type</td></tr>';
      return;
    }
    if (badge) badge.textContent = rows.length + ' rows';
    const maxC = Math.max(...rows.map(r => r.Incident_Count || 0), 1);
    tbody.innerHTML = rows.map((r, i) => {
      const pct = Math.round((r.Incident_Count || 0) / maxC * 100);
      const desc = r.Request_Description || '—';
      const grp  = r.AssignedGroup || '—';
      return `<tr>
        <td style="color:var(--dim);font-family:monospace;font-size:.7rem">${i+1}</td>
        <td style="font-size:.78rem;white-space:normal;line-height:1.5;max-width:380px">${desc}</td>
        <td><span class="chip c-blue" style="font-family:monospace">${r.Incident_Count}</span></td>
        <td style="font-size:.74rem;color:var(--muted);max-width:220px;white-space:normal">${grp}</td>
        <td style="min-width:90px"><div class="mbar"><div class="mbar-bg"><div class="mbar-fill"
          style="width:${pct}%;background:${accentColor}"></div></div></div></td>
      </tr>`;
    }).join('');
  }

  // Render all three description tables
  renderDescTable('rkm-desc-tbody', 'rkm-desc-badge', d.kb_article_rows, 'var(--purple)');
  renderDescTable('ke-desc-tbody',  'ke-desc-badge',  d.ke_desc_rows,    'var(--orange)');
  renderDescTable('pi-desc-tbody',  'pi-desc-badge',  d.pi_desc_rows,    'var(--cyan)');
