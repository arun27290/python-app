...
SLA_COLORS = {"Met": GREEN, "Within SLA": GREEN, "SLA Met": GREEN,
              "Breached": RED, "Pending": YELLOW, "Exempt": MUTED}
...
def make_donut(labels, values, colors=None, title="", size=(4.2, 3.4)):
...
def make_stacked_bar(labels, series, title="", ylabel="", size=(7, 3.5)):
    ...
    for name, vals, col in series:
        ...
        ax.bar(x, vals_arr, bottom=bottoms, label=name, color=col,
...
def analyse(df_raw):
    ...
    # date reference for Month/DayOfWeek
    ...
    hop_grp_alerts  = make_grp_alerts("Group_Transfers", 5, ">",
    ...
    kb_no_kb_rows    = []
    kb_summary_data  = {}
    kb_article_rows  = []      # NEW: KB-ID master table
    ...
    if kb_available:
        ...
        # ── KB Article Pivot Table ────────────────────────────────────────────
        # Logic: treat each row in RKM data as one incident tagged to a KB ID.
        # Raw description format:  "KBxxxxxx: some definition text here"
        # Step 1 — parse every RKM row's description into (KB_ID, Definition)
        # Step 2 — group by (KB_ID, Definition) → count incidents  ← pivot
        # Step 3 — produce 3 clean columns: KB Article | Definition | Count
        # Rows with NO parseable KB ID are collected separately as "No KB ID"

        def parse_kb_row(text):
            """Return (kb_id, definition) from a raw description string.
            Handles:  KB0001234: some text
                      KB0001234 - some text
                      KB0001234 some text     (no separator)
                      kb0001234:text          (lowercase)
                      KBxxxxxxx               (ID only, no definition)
                      free text without KB    (returns None, text)
            """
            if pd.isna(text) or str(text).strip() == "":
                return None, ""
            raw = str(text).strip()
            # find KB number anywhere in string
            m = re.search('(KB[0-9]{4,10})', raw, re.IGNORECASE)
            if not m:
                return None, raw  # no KB ID found
            kb_id = m.group(1).upper()
            # extract everything after the KB ID (strip leading separators)
            after = raw[m.end():].lstrip(" :-–—").strip()
            # if nothing after, try everything before the KB ID
            if not after:
                before = raw[:m.start()].strip().lstrip(" :-–—").strip()
                after = before if before else ""
            definition = after[:200]  # cap at 200 chars
            return kb_id, definition

        # Apply parser to every RKM row
        rkm_work = rkm_df.copy()
        rkm_work[["_p_kb_id", "_p_def"]] = pd.DataFrame(
            rkm_work["Request_Desc01"].apply(parse_kb_row).tolist(),
            index=rkm_work.index
        )

        # ── Pivot: group by (KB_ID, Definition) → count ───────────────────────
        has_kb_id = rkm_work["_p_kb_id"].notna()
        kb_pivot  = rkm_work[has_kb_id].copy()
        no_id_ct  = int((~has_kb_id).sum())   # rows with no KB ID in description

        kb_id_rows = []
        if len(kb_pivot) > 0:
            # Normalise definition: strip extra spaces, truncate for grouping key
            # Normalise for grouping: lowercase + collapse whitespace
            # (so 'Auth failure' and 'auth failure' group together)
            kb_pivot["_p_def_norm"] = (
                kb_pivot["_p_def"]
                .str.strip()
                .str.lower()
                .str.replace(r"\s+", " ", regex=True)
            )
            # Keep a display version (original case, first occurrence wins)
            kb_pivot["_p_def_display"] = kb_pivot["_p_def"].str.strip()
            # Group by (KB_ID, Definition) — this is the pivot
            # First pass: count + groups per (KB_ID, normalised_def)
            agg_dict = {"Incident_Count": ("_p_kb_id", "count")}
            if F["AssignedGroup"]:
                agg_dict["Groups"] = ("AssignedGroup",
                    lambda x: " · ".join(f"{g}({c})" for g,c in x.value_counts().items()))
            # Add display definition (take first non-empty value per group)
            agg_dict["Definition"] = ("_p_def_display",
                lambda x: next((v for v in x if v and str(v).strip()), "—"))

            pivot = (
                kb_pivot
                .groupby(["_p_kb_id", "_p_def_norm"], dropna=False)
                .agg(**agg_dict)
                .reset_index()
                .rename(columns={"_p_kb_id": "KB_Article"})
                .drop(columns=["_p_def_norm"])
                .sort_values("Incident_Count", ascending=False)
            )

            for _, row in pivot.iterrows():
                defn = str(row["Definition"]).strip() if pd.notna(row["Definition"]) else ""
                kb_id_rows.append({
                    "KB_Article":     row["KB_Article"],
                    "Definition":     defn[:180] + ("…" if len(defn) > 180 else "") if defn else "—",
                    "Incident_Count": int(row["Incident_Count"]),
                    "Groups":         row.get("Groups", "—") if F["AssignedGroup"] else "—",
                })

        # Append a summary row for incidents with no KB ID in description
        if no_id_ct > 0:
            kb_id_rows.append({
                "KB_Article":     "(No KB ID in description)",
                "Definition":     "RKM Solution tagged but no KBxxxxxx pattern found in description",
                "Incident_Count": no_id_ct,
                "Groups":         "—",
            })

        kb_article_rows = kb_id_rows
        log.info("KB pivot: %d unique (KB_ID, Definition) combinations, %d rows no KB ID",
                 len([r for r in kb_id_rows if r["KB_Article"] != "(No KB ID in description)"]),
                 no_id_ct)

    # ── KB charts ─────────────────────────────────────────────────────────────
    kb_charts = {}
    if kb_available and tagging_summary:
        kb_charts["tagging_donut"] = make_donut(
        ...
        if tagging_grp_rows:
            ...
            kb_charts["tagging_stacked"] = make_stacked_bar(
                g_names,
                [
        ...
        kb_charts = {k: v for k, v in kb_charts.items() if v}

    # ── generate all charts ───────────────────────────────────────────────────
    charts = {}
    charts["pri"]       = make_donut(pri_labels, pri_data,
                                      colors=[PRI_COLORS.get(l, PALETTE[i]) for i,l in enumerate(pri_labels)],
                                      title="Priority Distribution")
    charts["sla_donut"] = make_donut(sla_labels, sla_data,
                                      colors=[SLA_COLORS.get(l, PALETTE[i]) for i,l in enumerate(sla_labels)],
                                      title="SLA Status")
    charts["grp"]       = make_hbar(grp_labels[:20], grp_data[:20], color=BLUE,
                                     title="Incidents by AssignedGroup", xlabel="Count")
    charts["vol"]       = make_line(vol_labels, vol_data, color=BLUE,
                                     title="Monthly Incident Volume", ylabel="Incidents")
    charts["dow"]       = make_vbar(dow_labels, dow_data,
                                     colors=[PALETTE[i%len(PALETTE)] for i in range(len(dow_labels))],
                                     title="Incidents by Day of Week", ylabel="Count")
    charts["xfer"]      = make_vbar(xf_labels, xf_data, colors=[PURPLE]*len(xf_labels),
                                     title="Group Transfers Distribution", ylabel="Incidents")
    charts["sla_trend"] = make_line(sla_tr_labels, sla_tr_vals, color=GREEN,
                                     title="Monthly SLA Compliance %", ylabel="SLA %", pct=True)
    charts["sla_pri"]   = make_hbar_colored(sla_pri_labels, sla_pri_vals,
                                             title="SLA % by Priority", xlabel="SLA Met %")
    charts["mttr_grp"]  = make_hbar(mttr_grp_labels, mttr_grp_vals, color=YELLOW,
                                     title="MTTR by Group (avg hrs)", xlabel="Hours")
    charts["ci"]        = make_hbar(ci_labels[:15], ci_data[:15], color=CYAN,
                                     title="Top HPD_CI by Incident Count", xlabel="Count")
    charts["svc"]       = make_donut(svc_labels, svc_data, title="Service Type Mix")
    if pri_heat:
        charts["heatmap"] = make_heatmap(pri_heat["months"], pri_heat["priorities"],
                                          pri_heat["values"])
    # filter None charts
    ...
    return {
        ...
        "kb_article_rows":  kb_article_rows,
...
/* HEADER */
.hdr{position:sticky;top:0;z-index:500;background:rgba(7,9,15,.96);
...
<div id="dash">
  ...
  <section class="page on" id="pg-ov">
    ...
    <div class="al-card">
      ...
      <div class="al-body" id="hop-body"><div class="al-none">No incidents have transfers &gt; 5 ✓</div></div>
    </div>
    <!-- Aging > 30d -->
    <div class="al-card">
  ...
  <section class="page" id="pg-kb">
    ...
    <!-- Untagged incidents -->
    <div class="full al-card">
      <div class="al-hdr" style="background:rgba(255,79,106,.06);border-left:3px solid var(--red);cursor:default">
        <span style="font-size:1rem">🚫</span>
        <div class="al-info">
          <div class="al-title">Untagged Incidents — No RKM Solution, Known Error or Problem Investigation (first 300)</div>
          <div class="al-sub">Incidents where Request_Type01 does not match any of the three tag types</div>
        </div>
        <span class="al-badge" id="untag-badge" style="background:rgba(255,79,106,.15);color:var(--red)">0</span>
      </div>
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search…" oninput="filterTbl(this,'untag-tbody')"/>
        <button class="export-btn" onclick="exportCSV('untag-tbody','untagged_incidents')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead id="untag-thead"></thead><tbody id="untag-tbody"></tbody></table></div>
    </div>
    ...
    <!-- KB Article Master Table (NEW) -->
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--green);cursor:default">
        <span style="font-size:1rem">🔑</span>
        <div class="al-info">
          <div class="al-title">KB Article Master Table — KB ID · Description · Incident Count · Groups</div>
          <div class="al-sub">Each unique KB article found in Request Description01, with its description and usage</div>
        </div>
        <span class="al-badge" id="kb-art-badge" style="background:rgba(48,217,136,.12);color:var(--green)">0 KB IDs</span>
      </div>
      <div class="tbl-toolbar">
        <input class="search-box" placeholder="Search KB ID or description…" oninput="filterTbl(this,'kb-art-tbody')"/>
        <button class="export-btn" onclick="exportCSV('kb-art-tbody','kb_article_master')">⬇ CSV</button>
      </div>
      <div class="tbl-wrap"><table><thead><tr>
        <th>#</th>
        <th>KB Article</th>
        ...
        <th>AssignedGroups</th>
        <th>Volume</th>
      </tr></thead><tbody id="kb-art-tbody"></tbody></table></div>
    </div>
    ...
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--red);cursor:default">
        <span style="font-size:1rem">❌</span>
        <div class="al-info"><div class="al-title">Incidents Without KB Article Tag (first 200)</div></div>
      </div>
      <div class="tbl-toolbar">
...
function setImg(divId, src){const d=$(divId); if(d&&src)d.innerHTML='<img src="'+src+'" class="chart-img"/>';}
...
function buildKPIs(d){
  ...
  ov.forEach(o=>{
    ...
    if(d.charts[o.key]) setImg('ov-'+o.key, d.charts[o.key]);
  });
}
...
function grpAlertHTML(grpList){
  ...
  return grpList.map(g=>{
    ...
    const tbody=g.rows.map(r=>'<tr>'+cols.map(c=>{
      ...
      if(c==='SLAStatus'){const SLAM={'Met':'#30d988','Breached':'#ff4f6a','Pending':'#ffc240'};const sc=SLAM[v]||'#7b8db0';return `<td><span class="chip" style="background:${sc}22;color:${sc}">${v}</span></td>`;}
      return `<td>${v}</td>`;
    }).join('')+'</tr>').join('');
...
function buildCharts(d){
  // trends
  setImg('c-vol',d.charts.vol);
  setImg('c-dow',d.charts.dow);
  setImg('c-xfer',d.charts.xfer);
  setImg('c-sla-trend',d.charts.sla_trend);
  if(d.charts.heatmap){$('heatmap-card').style.display='';setImg('c-heatmap',d.charts.heatmap);}
  // sla
  setImg('c-sla-pri',d.charts.sla_pri);
  setImg('c-mttr-grp',d.charts.mttr_grp);
  // ci
  setImg('c-ci',d.charts.ci);
  setImg('c-svc',d.charts.svc);
}
...
function buildKB(d){
  ...
  // Tagging charts
  if(d.kb_charts){
    setImg('c-tag-donut',   d.kb_charts.tagging_donut);
    setImg('c-tag-stacked', d.kb_charts.tagging_stacked);
  }

  // Tagging by group table
  ...
  // Untagged incidents
  const ut = d.untagged_rows||[];
  $('untag-badge').textContent = ut.length;
  if(ut.length){
    const cols=Object.keys(ut[0]);
    $('untag-thead').innerHTML='<tr>'+cols.map(c=>`<th>${c.replace(/_/g,' ')}</th>`).join('')+'</tr>';
    $('untag-tbody').innerHTML=ut.map(r=>'<tr>'+cols.map(c=>`<td style="font-size:.74rem">${r[c]||'—'}</td>`).join('')+'</tr>').join('');
  }
  ...
  // KB Article Pivot Table — 3 clean columns: KB Article | Definition | Count
  const arts = d.kb_article_rows||[];
  const realArts = arts.filter(a=>a.KB_Article!=='(No KB ID in description)');
  const noIdRow  = arts.find(a=>a.KB_Article==='(No KB ID in description)');
  $('kb-art-badge').textContent = realArts.length+' unique KB Articles';
  const maxArt = arts.length ? Math.max(...arts.map(a=>a.Incident_Count)) : 1;
  $('kb-art-tbody').innerHTML = arts.map((a,i)=>{
    const pct   = Math.round(a.Incident_Count/maxArt*100);
    const isNone = a.KB_Article==='(No KB ID in description)';
    const kbCol = isNone
      ? `<span class="chip c-breach" style="font-size:.72rem">${a.KB_Article}</span>`
      : `<span style="font-weight:800;color:var(--green);font-family:monospace;font-size:.82rem">${a.KB_Article}</span>`;
    const defStyle = isNone
      ? 'font-size:.72rem;color:var(--dim);font-style:italic'
      : 'font-size:.76rem;max-width:340px;white-space:normal;line-height:1.4';
    return `<tr>
      <td style="color:var(--dim);font-family:monospace;font-size:.72rem">${i+1}</td>
      <td>${kbCol}</td>
      <td style="${defStyle}">${a.Definition}</td>
      <td><span class="chip c-blue" style="font-family:monospace;font-size:.8rem">${a.Incident_Count}</span></td>
      <td style="font-size:.72rem;color:var(--muted);max-width:180px">${a.Groups||'—'}</td>
      <td style="min-width:100px"><div class="mbar"><div class="mbar-bg"><div class="mbar-fill" style="width:${pct}%;background:${isNone?'var(--red)':'var(--green)'}"></div></div></div></td>
    </tr>`;
  }).join('');

  // No KB incidents
  if(d.kb_no_kb_rows&&d.kb_no_kb_rows.length){
    ...
    $('kb-no-tbody').innerHTML=d.kb_no_kb_rows.map(r=>
      '<tr>'+cols.map(c=>`<td style="font-size:.74rem">${r[c]||'—'}</td>`).join('')+'</tr>').join('');
  }
}
