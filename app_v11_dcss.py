...
def make_donut(labels, values, colors=None, title="", size=(4.2, 3.4)):
    if not labels or not values or sum(values) == 0:
        return None
    # Build full color list first (before filtering zeros)
    all_cols = colors if colors else [PALETTE[i % len(PALETTE)] for i in range(len(labels))]
    # Keep ALL entries in legend (including zero-count ones) but only plot non-zero wedges
    # This ensures color mapping stays correct even when some categories have 0 incidents
    legend_patches = [mpatches.Patch(color=c, label=f"{l} ({v})")
                      for l, v, c in zip(labels, values, all_cols)]
    # Filter to non-zero for the actual pie (matplotlib skips zero wedges and shifts colors)
    nz = [(l, v, c) for l, v, c in zip(labels, values, all_cols) if v > 0]
    if not nz:
        return None
    ...
    ax.set_facecolor(BG)
    ax.pie(nz_values, colors=nz_cols, startangle=90,
    ...
    # Legend shows ALL categories (including zero) with correct colors
