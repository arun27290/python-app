...
def analyse(df_raw):
    ...
    tagging_summary  = {}      # NEW: RKM Solution / Known Error / Problem Solution / Untagged
    ...
    PROB_SOL     = {"problem solution","problemsolution","problem_solution","ps","problem sol"}
    ...
    if kb_available:
        ...
        tagging_summary = {
            "labels":  ["RKM Solution", "Known Error", "Problem Solution", "Untagged"],
        ...
        # ── NEW: KB Article master table ──────────────────────────────────────
        # For each unique KB ID: KB ID, description, incident count, groups
        kb_id_rows = []
        kb_id_src  = rkm_df[rkm_df["_kb_id"].notna()].copy()
        for kb_id, kb_grp in kb_id_src.groupby("_kb_id"):
            # pick the most common description for this KB ID
            descs = kb_grp["_kb_desc"].dropna()
            desc  = descs.value_counts().index[0] if len(descs) > 0 else "—"
            desc  = desc[:120] + "…" if len(desc) > 120 else desc
            groups_using = []
            if F["AssignedGroup"]:
                g_counts = kb_grp.groupby("AssignedGroup").size().sort_values(ascending=False)
                groups_using = [f"{g} ({c})" for g, c in g_counts.items()]
            kb_id_rows.append({
                "KB_ID":        kb_id,
                "Description":  desc if desc else "—",
                "Incident_Count": len(kb_grp),
                "AssignedGroups": " · ".join(groups_using) if groups_using else "—",
            })
        kb_id_rows.sort(key=lambda x: x["Incident_Count"], reverse=True)
        ...
        if tagging_grp_rows:
            ...
            kb_charts["tagging_stacked"] = make_stacked_bar(
                ...
                [
                    ...
                    ("Problem Solution",[r["ps"]       for r in tagging_grp_rows], CYAN),
  ...
  <section class="page" id="pg-kb">
    ...
    <div class="full al-card">
      <div class="al-hdr" style="border-left:3px solid var(--purple);cursor:default">
        <span style="font-size:1rem">🏷</span>
        <div class="al-info">
          <div class="al-title">Tagging by AssignedGroup — RKM Solution · Known Error · Problem Solution · Untagged</div>
      ...
      <div class="tbl-wrap"><table><thead><tr>
        ...
        <th style="color:var(--cyan)">Problem Solution</th>
    ...
    <div class="full al-card">
      <div class="al-hdr" style="background:rgba(255,79,106,.06);border-left:3px solid var(--red);cursor:default">
        <span style="font-size:1rem">🚫</span>
        <div class="al-info">
          <div class="al-title">Untagged Incidents — No RKM Solution, Known Error or Problem Solution (first 300)</div>
    ...
    <div class="full al-card">
      ...
      <div class="tbl-wrap"><table><thead><tr>
        <th>KB Article ID</th><th>Description</th><th>Incident Count</th><th>AssignedGroups</th><th>Volume</th>
...
function buildKB(d){
  ...
  $('kb-kpis').innerHTML=[
    ...
    {l:'Problem Solution', v:s.total_ps||0,        c:'var(--cyan)',   i:'🔷', sub:'Problem Solution tagged'},
  ...
  $('kb-art-badge').textContent = arts.length+' KB IDs';
  ...
  $('kb-art-tbody').innerHTML = arts.map((a,i)=>{
    ...
    return `<tr>
      <td style="font-weight:800;color:var(--green);font-family:monospace">${a.KB_ID}</td>
      <td style="font-size:.74rem;max-width:280px;white-space:normal">${a.Description}</td>
      <td><span class="chip c-blue" style="font-family:monospace">${a.Incident_Count}</span></td>
      <td style="font-size:.73rem;color:var(--muted);max-width:200px">${a.AssignedGroups}</td>
      <td><div class="mbar"><div class="mbar-bg"><div class="mbar-fill" style="width:${pct}%;background:var(--green)"></div></div></div></td>
    </tr>`;
  }).join('');
